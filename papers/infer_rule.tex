...
This is the type of a sigma \sigl{x, f}.

\begin{itemize}
  \item{Pairs are created with cons (\texttt{::})}
  \item{Notation: \sigl{x_{1}, \cdots x_{n}} denotes \sigl{x_{1}, \text{\sigl{x_{2}, \cdots}}}.}
  \item{Can nest sigmas}
  \item{Cons can be partially applied. This requires a special case for infer.}
  \item{No explicit \texttt{fst} and \texttt{snd} combinators:}
    \begin{itemize}
      \item{\sigl{x, f} accepts a ``projector'' (\(\pi\)). \(\esigl{x, f} \(\pi\) \mapsto 
    \end{itemize}
I use the  to denote 
