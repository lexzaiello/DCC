...
\begin{frame}{Very Dependent Types}
  \begin{itemize}
    ...
    \item \(x : F\ x\): very dependent types, such as this one featured in Altenkirch et al. \cite{altenkirch} might be useful.
    \item Since our sigma terms encode application as data, we can easily traverse the ``context''.
    \item \leane{Σ t_in t_out : Type}
    \item To infer domain / codomain: \leane{(((f : Σ Tα Tβ) (x : α)) : ((T ::[x, f]) π))}
      \begin{itemize}
        \item Problem: to project either component, we must know \(\alpha\) and \(\beta\).
      \end{itemize}
