...
inductive IsStep : Expr → Expr → Prop
  ...
  | const  : IsStep ($ const, _α, _β, x, y) x
  | pi     : IsStep ($ ($ Pi, l), x) ($ l, x)
  | prod   : IsStep ($ ($ Prod, l), x) ($ l, x)
  | left   : IsStep f f'
    → IsStep ($ f, x) ($ f', x)
  | right  : IsStep x x'
    → IsStep ($ f, x) ($ f, x')
...
So what's id's type then?

id : ∀ (α : Type), α → α

inner pi receives α
Pi (both Ty _ _ (Pi ::[nil, nil]

Question: how do we handle contexts? with both, almost certainly.

What if we do it in reverse?
second argument gets substituted first,
then first gets its value, and it feels like
the output will mention a more complex term than the input.

id α x, x is a term. T_OUT GOES SECOND.
...
α_in receives ($ nil, (α : Ty)) and then (α : Ty)
($ nil, (α : Ty)) : 
so α_in = ($ const', ?, Ty,

id α x : ::[($ nil, α, x), ($ nil, α, x)]
-/
...
inductive ValidJudgment : Expr → Expr → Prop
  | cons : ValidJudgment xs β
    → ValidJudgment x ($ Pi, ::[α, β])
    → ValidJudgment ::[x, xs] ($ Prod, ::[($ Pi, ::[α, β]), β])
  | Pi   : ValidJudgment Pi   ($ Pi, ::[($ nil, Ty), Prod])
  | Prod : ValidJudgment Prod ($ Pi, ::[($ nil, Ty), Prod])
  | app  : ValidJudgment fn ($ Pi, t)
    → ValidJudgment t_in ($ Pi, ::[($ nil, Ty), β])
    → ValidJudgment t_out ($ Pi, ::[β, α])
    → ValidJudgment arg ($ fst, α, t_out, t_in, ($ t, arg))
    → ValidJudgment ($ fn, arg) ($ snd, α, t_out, t_in, ($ t, arg))
