\input{./papers/template2.tex}
\usepackage[margin=0.75in]{geometry}
\usepackage{fitch}
\usepackage{bbm}
\usepackage{stmaryrd}
\usepackage{hyperref}
\usepackage{mathrsfs}
\usepackage{mathtools}
\usepackage{rsfso}
\usepackage{graphicx}
\usepackage[
  backend=biber,
  style=alphabetic,
  sorting=ynt
]{biblatex}
\addbibresource{./refs.bib}


\author{Alexandra Aiello}
\date{December 19, 2025}
\title{The Dependent Arithmetic Machine}

\begin{document}

This is the type of a pair \sigl{x, f}. Pairs are created with cons (\texttt{::}).
Cons can be partially applied. Also, pairs can be nested.
I use the notation \sigl{\(x_{1}, \cdots x_{n}\)} to denote

\begin{equation}
  \frac{
    
  }{
    
  }
\end{equation}

These are the types of the core DCC combinators.

Note that I am denoting types like \(\forall (x : \alpha), \beta(x)\) in point-free notation as \(\beta\), since
there are no \(\Pi\) binders in the DCC. This is explained more in the application inference rule.

I am using standard \(\Pi\) and \(\times\) notation here for legibility, and since I have not yet completed all the DCC tyeps in the latest, most sound version. Later in this overview, I outline the challenges I encountered, and the progress I have made towards finishing them.

\begin{equation}
\frac{}{
  \text{id}(m) : \Pi (\alpha : \type{m}) (x : \alpha), \alpha
}\\
\end{equation}

\begin{equation}
\frac{}{
  \text{nil}(m) : \Pi (\alpha : \type{m}) (x : \alpha), \type{m}
}\\
\end{equation}

\begin{equation}
\frac{}{
  \text{const'}(m,  n) : \Pi (\alpha : \type{m}) (\beta : \type{n}) (x : \alpha) (y : \beta), \alpha
}\\
\end{equation}

\begin{equation}
\frac{}{
  \text{const}(m,  n) : \Pi (\alpha : \type{m}) (\beta : \alpha \rightarrow \type{n}) (x : \alpha) (y : \beta\ x), \alpha
}\\\\
\end{equation}

\newpage

The rule for \texttt{both} is quite long, so I have written it in fully applied style, as it would not fit on the page otherwise.

\begin{equation}
  \frac{\Gamma, \vdash (\alpha : \type{m}),
    (\beta : \alpha \rightarrow \type{n}),
    (\gamma : \alpha \rightarrow \type{o}),
    (f : \Pi (x : \alpha), \beta(x)),
    (g : \Pi (x : \alpha), \gamma(x)),
    (x : \alpha)}{
    \text{both}(m, n, o, \alpha, \beta, \gamma, f, g, x) : (\beta(x) \times \gamma(x))
}
\end{equation}

\begin{equation}
  \frac{}{
    \type{n} : \type{n + 1}
  }
\end{equation}

\begin{equation}
  \frac{}{
    
  }
\end{equation}

What's left?

\begin{itemize}
  \item{cons}
  \item{app}
  \item{eq, but if I don't get to it, that's fine}
\end{itemize}

\end{document}
