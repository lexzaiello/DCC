...
\begin{frame}{Central Thesis: The Sigma-Curry Correspondence}
  \begin{itemize}
    ...
    \item I demonstrate \hyperlink{equivfst}{\beamergotobutton{here}} that \texttt{Sigma.snd} projection is equivalent to function application
...
\begin{frame}{TODOs}
  \begin{itemize}
    \item \sout{Should we internalize \(\pi\) projection in \sigl{a, b}, or should we have \sigl{fst, snd} combinators?} \textbf{Yes!}
      \begin{itemize}
...
\begin{frame}[fragile]{All Function Applications are \(=_{\beta}\) Sigma Projection}
  \begin{leancode}
/-
  (f x) β= (::[x, f] (id t_f))
-/
