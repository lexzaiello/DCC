...
\begin{frame}[fragile]{Central Thesis: Sigma-Substitution Correspondence}
  \begin{figure}
...
Sigma.fst.{u, v} {α : Type u} {β : α → Type v}
  (self : Sigma β) : α
\end{minted}
  \caption{The dependent \(K\) combinator has the same type as \leane{Sigma.fst}. Hypothesis: \leane{Sigma.snd} projection corresponds to substitution under \(\forall\) binders.}
\end{figure}
\end{frame}
...
%  \begin{itemize}
%    \item I hypothesize that \leane{{Sigma.fst, Sigma.snd, Sigma.uncurry, Pair.fst}} constitute a turing-complete dependently-typed combiantor calculus.
%    \item We can simulate substitution in Lean's \leane{Sigma.snd} by applying \leane{Sigma.fst} as an argument to our \leane{Sigma.snd}.
%    \item By defining reduction for the combinators in terms of a single sigma argument, ``uncurried'' combinators become well-behaved curried functions.
%  \end{itemize}
%\end{frame}
...
\begin{frame}[fragile]{Inference Rule: Application}
  \begin{aligned}
    ...
    f : \sigle{\alpha, \beta},
    x : \alpha}{\Gamma \vdash (f\ x) : \beta\ \sigle{x, \alpha}}
  \end{aligned}
\end{frame}
